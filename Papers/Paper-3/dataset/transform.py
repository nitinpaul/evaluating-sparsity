...
def random_crop(im, size, pad_size=0):
    ...
    if pad_size > 0:
        im = zero_pad(im, pad_size=pad_size)
...
def random_sized_crop(im, size, area_frac=0.08, max_iter=10):
    ...
    for _ in range(max_iter):
        ...
        if h_crop <= h and w_crop <= w:
            ...
            return cv2.resize(im_crop, (size, size), interpolation=cv2.INTER_LINEAR).astype(np.float32)
...
def lighting(im, alpha_std, eig_val, eig_vec):
    ...
    rgb = np.sum(eig_vec * alpha * eig_val, axis=1)  # More efficient calculation
